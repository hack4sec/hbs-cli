...
class Test_HashlistsLoaderThread(CommonIntegration):
    ...
    def test_run(self, alg_id, have_salt, hashes):
        ...
        assert 1 == self.db.fetch_one("SELECT 1 FROM hashes WHERE id = 1 AND cracked = 0 AND password=''")
        assert 1 == self.db.fetch_one("SELECT 1 FROM hashes WHERE id = 2 AND cracked = 0 AND password=''")
